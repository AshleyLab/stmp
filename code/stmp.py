...
resources_path= os.path.join(cur_path, os.path.pardir, 'data', 'resources')
...
def parse_args(args):
	...
	parser.add_argument('--test', action='store_true', help='test STMP annotation, tiering, and pgx on the included VCF file (very small Genome in a Bottle subset)')
	...
	annotate_group.add_argument('--database_file', help='path to SQLite database file (default: code/../data/db/annotationDB.sqlite)', default=os.path.join(cur_path, '..', 'data', 'db' ,'annotationDB.sqlite'))
	...
	tiering_group.add_argument('--suppress_incidentals', action='store_true', help='suppress incidental findings (all target gene lists with "clinical" in the name will be ignored when tiering')
...
# stub code to run main function
